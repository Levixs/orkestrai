...
- When Apple signing secrets are absent, unset all signing variables and set `CSC_IDENTITY_AUTO_DISCOVERY=false`.
...
- Unsigned macOS builds detect updates but use manual download. In-place macOS update requires Apple signing and notarization secrets.
