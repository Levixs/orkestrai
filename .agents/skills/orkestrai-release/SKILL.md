...
Run the complete release from the `beeblock/pantheon` repository and do not stop while required GitHub Actions jobs are active.
...
- Add the newest pt-BR section to `CHANGELOG.md`.
- Add equivalent changelog entries, in the same position and structure, to `src/lib/i18n/docs/pt-BR.ts`, `en.ts`, and `es.ts`.
...
```bash
gh run list --repo beeblock/pantheon --workflow "Release Desktop" --limit 3
gh run watch <run-id> --repo beeblock/pantheon --exit-status
```

Confirm all five jobs: tag validation, macOS, Windows, Linux, and verified publication. Do not report completion while the run is queued or in progress.
...
```bash
gh run view <run-id> --repo beeblock/pantheon --log-failed
```
...
If the release is already public, stop recovery and create a new patch version. Never mutate the public version.
