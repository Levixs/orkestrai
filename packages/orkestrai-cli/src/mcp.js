...
const TOOLS = [
  ...
  { name: 'task_add', description: 'Cria tarefa; com assignee ja despacha para o agente.', inputSchema: { type: 'object', properties: { title: { type: 'string' }, assignee: { type: 'string' }, note: { type: 'string', description: 'Nota de spec (id ou titulo)' } }, required: ['title'] } },
...
async function callTool(bridge, findFreePort, selfAgent, name, args = {}) {
  switch (name) {
    ...
    case 'task_add':
      return bridge('POST', '/api/agent-room/bridge/tasks', { title: args.title, assignee: args.assignee, note: args.note, from: selfAgent });
    case 'task_done':
