...
describe('mcp-fanout', () => {
  ...
  it('writes a command-based server into every standard JSON path and opencode.json', async () => {
    await fanOutMcpServer(workingDir, 'context7', { command: 'npx', args: ['-y', 'context7-mcp'], env: { API_KEY: 'x' } });

    for (const relativePath of ['.cursor/mcp.json', '.cline/mcp.json', '.devin/mcp_config.json', '.agents/mcp_config.json']) {
      const config = JSON.parse(await readFile(join(workingDir, relativePath), 'utf8'));
      expect(config.mcpServers.context7).toEqual({ command: 'npx', args: ['-y', 'context7-mcp'], env: { API_KEY: 'x' } });
    }
  ...
  it('writes a remote (url) server as a bare url entry, and as type "remote" for opencode', async () => {
    await fanOutMcpServer(workingDir, 'remote-server', { url: 'https://example.com/mcp' });

    const cursor = JSON.parse(await readFile(join(workingDir, '.cursor/mcp.json'), 'utf8'));
    expect(cursor.mcpServers['remote-server']).toEqual({ url: 'https://example.com/mcp' });

    const opencode = JSON.parse(await readFile(join(workingDir, 'opencode.json'), 'utf8'));
    expect(opencode.mcp['remote-server']).toEqual({ type: 'remote', url: 'https://example.com/mcp', enabled: true });
  });

  it('merges with existing entries instead of overwriting them', async () => {
    await fanOutMcpServer(workingDir, 'first', { command: 'npx', args: ['first-mcp'] });
    await fanOutMcpServer(workingDir, 'second', { command: 'npx', args: ['second-mcp'] });
  ...
  it('removes a fanned-out server from every path without touching siblings', async () => {
    await fanOutMcpServer(workingDir, 'keep', { command: 'npx', args: ['keep-mcp'] });
    await fanOutMcpServer(workingDir, 'drop', { command: 'npx', args: ['drop-mcp'] });
