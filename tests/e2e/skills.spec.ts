...
test.describe('marketplace de skills', () => {
  test('busca no skills.sh, instala no workspace e remove', async ({ page, request }) => {
    ...
    try {
      ...
      await expect(page.locator('.installed-item', { hasText: 'orkestrai-bridge' })).toBeVisible();
      ...
      const installedSkill = page.locator('.installed-item', { hasText: 'web-design-guidelines' });
      ...
      await expect(page.locator('.installed-item')).toHaveCount(1);
      await expect(page.locator('.installed-item', { hasText: 'orkestrai-bridge' })).toBeVisible();
    } finally {
