...
export const DOCS_PT: DocsCatalog = {
  ...
  useCases: [
    ...
    },
    {
      ...
      body: 'Abra Design na barra do Canvas e escolha Três direções completas de UI. Informe objetivo, público, plataforma, destino do código, restrições, referências e se Light e Dark são obrigatórios. O Orkestrai cria um grupo rastreável com uma spec vinculada, cinco etapas no Kanban e três documentos Design nativos e editáveis: Clareza, Expressiva e Eficiente. Trabalhe manualmente ou delegue a primeira etapa a um líder que esteja rodando. Todo despacho inclui a spec vinculada completa, imagens, arquivos e links. Agentes consultam o contrato exato e produzem cada direção em poucos lotes transacionais, sem probes ou scripts de descoberta. Cada direção deve entregar estados responsivos, tokens tipados, componentes reutilizáveis, protótipo navegável, evidências de acessibilidade e qualidade e preview de código real do framework. Em documentos largos, navegue com trackpad, Mão (H), Espaço+arraste ou botão do meio e use Ajustar para enquadrar todos os frames. Compare marca, UX, esforço e risco técnico; uma pessoa precisa aprovar uma direção ou combinação explícita antes de aplicar código. Finalize validando a fonte aprovada contra um Portal ou dispositivo móvel e registrando o resultado ligado ao Git no Review Center.',
