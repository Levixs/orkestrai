...
export const DOCS_PT: DocsCatalog = {
  ...
  useCases: [
    ...
    },
    {
      ...
      body: 'Habilite o compartilhamento experimental, inicie uma sessão criptografada de ponta a ponta e escolha um convite para Navegador/celular ou App Orkestrai. O link web abre o PWA Remote instalável. O convite do app abre automaticamente o Orkestrai instalado; o convidado também pode usar Workspace → Entrar em workspace remoto e colar o convite. Ambos removem o segredo da URL antes da conexão e guardam uma chave de pareamento não extraível somente naquele dispositivo. Confira a impressão digital e escolha Leitor, Colaborador, Operador ou Administrador. Um Operador pode manter uma conversa sanitizada e rastreável com um agente e ditar uma mensagem ao líder usando o STT local do host; a resposta do chat só aparece quando corresponde à pergunta e à conversa exatas. Um Administrador também pode iniciar ou restaurar um agente offline. O terminal bruto é uma permissão separada, exclusiva de Administrador naquele dispositivo, desativada por padrão, limitada a um terminal, responsiva à tela atual, com limite de tráfego, criptografia e auditoria. Ela não libera navegação de arquivos, visualização de Portal ou dispositivo mobile nem edição do Canvas. Revogue o dispositivo ou encerre a sessão a qualquer momento e consulte comandos aceitos e rejeitados na auditoria.',
  ...
  changelog: [
    {
      ...
      items: [
        'A colaboração remota agora permite conversas rastreáveis vinculadas à pergunta e à sessão exatas em todos os providers registrados, ditado de mensagens ao líder pelo STT local do host, início ou restauração exclusivo para Administrador e um terminal bruto aprovado separadamente, desativado por padrão, responsivo no celular, limitado a uma sessão, com limite de tráfego, criptografia e auditoria. Abrir o terminal fecha a conversa antes de ocupar a tela inteira.',
