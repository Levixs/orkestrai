...
export const DOCS_PT: DocsCatalog = {
  ...
  sections: [
    ...
    },
    {
      ...
      body: `Adicione Dispositivo móvel pela barra do Canvas. Ele é um único node persistente do workspace; o Workbench lista e abre esse mesmo node com a mesma sessão e o mesmo estado do iOS Simulator. Em Macs Apple Silicon com Xcode instalado, o painel lista os runtimes locais, inicia ou conecta ao simulador, transmite a tela e envia toques, swipes, gestos de pinça, rotação, botão Home e entrada de texto. A tela inteira é ajustada automaticamente ao espaço disponível no Canvas e no Workbench; controles independentes permitem reduzir, ampliar, voltar ao ajuste automático ou usar 1:1 com scroll horizontal e vertical. Redimensionar o node ou painel e girar o dispositivo recalcula o enquadramento sem alterar as coordenadas de toque. A gaveta de ferramentas instala e abre apps do workspace, salva screenshots em .orkestrai/devices/screenshots, lê logs limitados e a árvore de acessibilidade e inspeciona ou altera permissões do simulador. Os agentes recebem as mesmas ações confinadas ao workspace pela CLI orkestrai device e pelas tools MCP, para reproduzir e documentar um fluxo mobile. O Orkestrai encerra apenas helpers e simuladores que ele iniciou; cada workspace admite um node e uma sessão, e sessões ociosas são limpas. Android aparece como uma etapa separada e ainda não é informado como disponível.`,
    },
    {
      id: 'notas',
  ...
  useCases: [
    ...
    {
      id: 'mobile-qa',
      title: 'Reproduzir e validar um fluxo no iOS',
      body: 'Adicione Dispositivo móvel ao Canvas ou abra-o pelo Workbench, conecte um iPhone ou iPad Simulator disponível e instale um app do workspace. O mesmo node persistente e a mesma sessão continuam disponíveis nas duas visualizações. A tela completa se ajusta ao painel por padrão; use zoom, 1:1 e scroll nos dois eixos para inspecionar detalhes. Você ou um agente pode tocar, arrastar, digitar, girar, alterar permissões, inspecionar a árvore de acessibilidade, capturar um screenshot e coletar logs limitados mantendo todos os artefatos dentro do projeto.',
      tags: ['iOS Simulator', 'QA mobile', 'CLI/MCP'],
    },
    {
      id: 'research-summary',
  ...
  changelog: [
    {
      ...
      items: [
        ...
        'Canvas e Workbench agora compartilham um node persistente de Dispositivo móvel e uma sessão de iOS Simulator por workspace em Macs Apple Silicon, com tela ao vivo, controles de toque e gestos, rotação, digitação, instalação e abertura de apps, screenshots, logs, árvore de acessibilidade, permissões, limpeza do ciclo de vida e comandos equivalentes na CLI orkestrai e no MCP. A tela completa se ajusta ao painel por padrão, com zoom independente, modo 1:1, recálculo ao redimensionar ou girar e scroll nos dois eixos. Android permanece como backend futuro explicitamente indisponível.',
      ],
    },
    {
