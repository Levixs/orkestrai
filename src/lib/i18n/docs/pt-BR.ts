...
export const DOCS_PT: DocsCatalog = {
  quickstart: [
    ...
    'Clique em qualquer agente disponível na barra inferior e arraste um retângulo no canvas — nomeie, escolha modelo/esforço se quiser e marque Líder se ele vai comandar o time.',
  ...
  sections: [
    ...
    },
    {
      ...
      body: `MCP é o padrão para dar ferramentas externas aos agentes (GitHub, Gmail, Figma, Drive, Postgres...). O JEITO FÁCIL: página Skills (barra lateral) → aba MCPs — pesquise na curadoria oficial ou no registry MCP e instale com um clique; se o servidor pedir chave/token, o app explica onde conseguir. Remotos instalam sem comando. AVANÇADO: lápis ao lado do workspace → "Servidores MCP". AUTOMÁTICO: o Orkestrai provisiona a própria ponte nos formatos de Claude/Kimi (.mcp.json), Codex (~/.codex/config.toml), OpenCode (opencode.json), Cursor (.cursor/mcp.json), Cline (.cline/mcp.json) e Antigravity (.agents/mcp_config.json), além das skills e do bloco preservado em AGENTS.md. Cada agente recebe as ações do canvas como tools tipadas no workspace correto.`,
    },
    {
      id: 'cli',
      title: 'CLI orkestrai (a ponte)',
      body: `Os agentes usam a CLI orkestrai para agir no canvas: list, ask, note read/write/edit/create, task list/columns/add/move/assign/done/archive/history, role show/write/edit, floor create/list/preview/land/remove, notify, recruit/dismiss/connect/reassign, portal, port, fs, run, say, clip, notes e portals. task columns devolve as etapas definidas por você; task add --column e task move permitem que líder e equipe respeitem qualquer processo, não apenas um kanban de software. Agentes que falam MCP recebem as mesmas ações como tools nativas via orkestrai mcp. O provisionamento da ponte é automático e o token fica em .orkestrai/workspace.json.`,
    },
    {
      id: 'atalhos',
      title: 'Atalhos',
      body: `⌘P paleta · ⌘K (ou Ctrl+K) buscar na documentação de qualquer tela · ⌘2 Central de Providers · ⌘⇧A próxima atenção · ⌘⇧T organizar · ⌘G agrupar · ⌘⇧G desagrupar · N nova nota · L conectar selecionados · Alt+1…9 focar terminal · Alt+Espaço ditado por voz · ⌘F buscar no terminal · ⌘Z desfazer · Backspace excluir. No desktop, o menu nativo Workspace abre Canvas, Central de Providers, novo workspace, Presets, Andares, Roles, Uso e Portas; Editar, Visualizar, Janela e Ajuda mantêm ações do sistema em macOS, Windows e Linux.`,
    },
  ],
  useCases: [
    {
    ...
    },
    {
      ...
      body: 'Use o provider que você já tem instalado e autenticado; o Orkestrai cuida do terminal, da ponte e da retomada da conversa. Claude, Codex, Kimi, OpenCode, Cursor, Antigravity e Cline aparecem na mesma barra quando disponíveis. Para uma campanha, identidade visual, pesquisa, conteúdo ou produto, nomeie os agentes pelo resultado esperado e combine um segundo provider apenas quando quiser revisão independente.',
      tags: ['7 providers', 'sem terminal', 'qualquer profissão'],
    },
    {
      id: 'setup-agent-provider',
      title: 'Preparar um provider de IA sem adivinhar comandos',
      body: 'Abra a Central de Providers para ver quais agentes este dispositivo já consegue usar. Expanda Claude, Codex, Kimi, OpenCode, Cursor, Antigravity ou Cline, siga a instalação indicada para seu sistema, conclua o login na CLI oficial e use Verificar novamente antes de voltar ao canvas.',
    ...
    },
    {
      ...
      body: 'Abra a Biblioteca de presets e escolha React, Next.js, SvelteKit, Svelar ou Laravel. O projeto nasce com líder, implementação, arquitetura e QA conectados, roles completas, skills para Claude/Codex, quadro e tarefa inicial. Salve o workspace como preset quando quiser duplicar e personalizar a receita.',
