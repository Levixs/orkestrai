...
export const DOCS_PT: DocsCatalog = {
  ...
  useCases: [
    ...
    },
    {
      ...
      body: 'Adicione um node Design nativo e abra no Modo Design do Canvas ou no Workbench. Monte paths vetoriais, máscaras, gradientes, frames responsivos com auto layout e assets de imagem reutilizáveis ou cole um SVG para transformar paths, transforms, estilos e gradientes em camadas nativas editáveis. Agrupe ou desagrupe a arte, selecione todas as camadas que usam a mesma cor, substitua cores correspondentes em sólidos e stops de gradiente e copie a seleção como SVG ou PNG. Em Variáveis, monte tokens tipados em coleções, defina modos Claro, Escuro, marca ou produto, reutilize valores por aliases e vincule-os às propriedades compatíveis pelo inspector. Conecte o documento ao Designer ou líder: o agente lê a revisão atual exata, aplica as mesmas operações tipadas de elementos, assets, guias, layout, variáveis, modos e vínculos e verifica o resultado enquanto seu editor atualiza ao vivo. Exporte a seleção ou a página aprovada em SVG, PNG, JPEG, WebP ou PDF. Documento, assets, thumbnails, tokens e histórico de revisões ficam no workspace e continuam pesquisáveis junto com tarefas, notas, arquivos, portais e o restante do time.',
