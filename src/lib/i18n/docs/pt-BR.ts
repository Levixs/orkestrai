...
export const DOCS_PT: DocsCatalog = {
  ...
  sections: [
    ...
    },
    {
      ...
      body: `Adicione Design pela barra do Canvas para criar um documento visual estruturado salvo em .orkestrai/designs dentro do projeto. Dê duplo-clique na prévia ou use expandir para abrir o Modo Design; o mesmo documento abre como item completo no Workbench. Desenhe frames, retângulos, elipses, textos e paths com a Caneta em qualquer tamanho. Na Caneta, clique para criar cantos, clique e arraste para criar tangentes e clique no primeiro ponto para fechar; finalize um path aberto com Enter ou duplo-clique. Com o path selecionado, arraste âncoras e alças Bézier, use Shift para restringir a alça a 45 graus, Alt para quebrar a tangência, duplo-clique em um segmento para inserir um ponto e converta pontos entre Canto e Suave nas propriedades. Use Shift para selecionar camadas, alinhar, distribuir, combinar por união, subtração, interseção ou exclusão e criar ou liberar máscaras. Empilhe fills e strokes sólidos ou com gradiente linear/radial; adicione sombras, blur, blend modes, réguas, guias persistentes e snap. Frames aceitam auto layout horizontal, vertical, com wrap ou grid, padding e gaps, enquanto constraints dos filhos respondem ao redimensionamento. Importe PNG, JPEG, WebP, GIF ou SVG por seletor, clipboard ou arraste e solte, reutilize o asset e exporte a página como SVG, PNG, JPEG, WebP ou PDF. Todos os ícones da barra têm tooltip e as setas abrem menus de opções. Delete remove o ponto ou as camadas selecionadas, Esc sai primeiro da edição do ponto e desfazer/refazer usa operações tipadas. Cada mutação registra revisão e histórico limitado; thumbnails vinculados à revisão mantêm prévias grandes leves no Canvas. Conecte o node Design ao líder ou especialista para que ele leia e edite o scene graph exato por tools MCP tipadas do Orkestrai. Edições humanas e de agentes usam controle otimista de revisão, atualizam os editores ao vivo e nunca exigem reescrita direta do JSON.`,
    },
    {
      id: 'presets',
  ...
  changelog: [
    {
      ...
      items: [
        'A barra do Modo Design agora exibe tooltips e menus acima do editor, e a Caneta ganhou edição Bézier direta: criar tangentes ao arrastar, mover âncoras e alças, dividir segmentos preservando a curva, converter pontos entre Canto e Suave e adicionar ou remover pontos com desfazer/refazer.',
