...
export const DOCS_EN: DocsCatalog = {
  ...
  sections: [
    ...
    },
    {
      ...
      body: `Add Design from the Canvas toolbar to create a structured visual document stored under .orkestrai/designs in the project. Double-click its preview or use expand to open Design Mode; the same document opens as a full Workbench item. Draw frames, rectangles, ellipses, text, and Pen paths at any size. With Pen, click for corners, click-drag for tangents, and click the first point to close; finish an open path with Enter or double-click. Select a path to drag anchors and Bezier handles, hold Shift to constrain a handle to 45 degrees, hold Alt to break tangency, double-click a segment to insert a point, and switch points between Corner and Smooth in Properties. Shift-select layers to align, distribute, combine with union, subtract, intersect, or exclude, and create or release masks. Stack solid or linear/radial gradient fills and strokes; add shadows, blur, blend modes, rulers, persistent guides, and snapping. Frames support horizontal, vertical, wrapping, and grid auto layout, padding and gaps, while child constraints respond when a frame is resized. Import PNG, JPEG, WebP, GIF, or SVG by picker, clipboard, or drag and drop, then reuse the asset or export the page as SVG, PNG, JPEG, WebP, or PDF. Every toolbar icon has a tooltip and arrow buttons open option menus. Delete removes the selected point or layers, Escape leaves point editing first, and undo/redo uses typed operations. Every mutation records a revision and bounded history; revision-bound thumbnails keep large Canvas previews light. Connect the Design node to a leader or specialist so they can read and edit the exact scene graph through typed Orkestrai MCP tools. Human and agent edits use optimistic revision checks, update open editors live, and never require direct JSON rewriting.`,
    },
    {
      id: 'presets',
      title: 'Team presets',
  ...
  useCases: [
    ...
    },
    {
      ...
      body: 'Add a native Design node and open it in Canvas Design Mode or Workbench. Build vector paths, masks, gradients, responsive auto-layout frames, and reusable image assets yourself, then connect the document to a Designer or leader. The agent reads the exact current revision, applies the same typed element, asset, guide, and layout operations, and verifies the result while your editor refreshes live. Export the approved page to SVG, PNG, JPEG, WebP, or PDF. The document, assets, thumbnails, and revision history stay in the workspace and remain searchable alongside tasks, notes, files, portals, and the rest of the team.',
  ...
  changelog: [
    {
      date: 'In development',
      items: [
        'Design Mode tooltips and dropdowns now render above the editor, and Pen has direct Bezier editing: drag to create tangents, move anchors and handles, split segments without changing their curve, convert Corner/Smooth points, and add or remove points with undo/redo.',
        'Expanded native Design Mode with editable Pen paths, boolean operations, masks, multiple solid or gradient paints, effects and blend modes; snapping, rulers, guides, alignment and distribution; responsive horizontal, vertical, wrapping, and grid auto layout; reusable image/SVG assets imported by picker, paste, or drop; SVG, PNG, JPEG, WebP, and PDF export; and revision-bound raster thumbnails for efficient large Canvas previews.',
