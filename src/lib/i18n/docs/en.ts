...
export const DOCS_EN: DocsCatalog = {
  ...
  useCases: [
    ...
    },
    {
      ...
      body: 'Add Mobile Device to the Canvas or open it from Workbench, attach an available iPhone or iPad Simulator, and install an app from the workspace. The same persistent node and session remain available in both views. The complete screen fits the pane by default; use zoom, 1:1, and two-axis scrolling to inspect details. You or an agent can tap, swipe, type, rotate, change permissions, inspect the accessibility tree, capture a screenshot, and collect bounded logs while keeping every artifact inside the project.',
