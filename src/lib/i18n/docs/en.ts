...
export const DOCS_EN: DocsCatalog = {
  ...
  useCases: [
    ...
    },
    {
      ...
      body: 'Add a native Design node and open it in Canvas Design Mode or Workbench. Build vector paths, masks, gradients, responsive auto-layout frames, and reusable image assets yourself, or paste an SVG to turn its paths, transforms, styles, and gradients into editable native layers. Group or ungroup artwork, select every layer that uses the same color, replace matching colors across solids and gradient stops, then copy a selection as SVG or PNG. In Variables, build typed tokens in collections, define Light, Dark, brand, or product modes, reuse values through aliases, and bind them to compatible layer properties from the inspector. Connect the document to a Designer or leader: the agent reads the exact current revision, applies the same typed element, asset, guide, layout, variable, mode, and binding operations, and verifies the result while your editor refreshes live. Export the approved selection or page to SVG, PNG, JPEG, WebP, or PDF. The document, assets, thumbnails, tokens, and revision history stay in the workspace and remain searchable alongside tasks, notes, files, portals, and the rest of the team.',
