...
export const DOCS_EN: DocsCatalog = {
  ...
  useCases: [
    ...
    },
    {
      ...
      body: 'Open Design in the Canvas toolbar and choose Three complete UI directions. Enter the objective, audience, platform, code target, constraints, references, and whether both Light and Dark are required. Orkestrai creates one traceable group with a linked spec, five Kanban stages, and three native editable Design documents: Clarity, Expressive, and Efficient. Work manually or delegate the first stage to a currently running leader. Every task dispatch includes the complete linked spec, images, files, and links. Agents consult the exact contract and produce each direction in a few transactional batches without probes or discovery scripts. Each direction must deliver responsive states, typed tokens, reusable components, a navigable prototype, accessibility and quality evidence, and a preview of real framework code. In wide documents, navigate with the trackpad, Hand (H), Space-drag, or middle mouse button and use Fit to frame every screen. Compare brand, UX, effort, and technical risk; a person must approve one direction or an explicit combination before code is applied. Finish by validating the approved source against a Portal or mobile device and recording the Git-linked result in Review Center.',
