  ...
  sections: [
    ...
    },
    {
      ...
      bullets: [
        ...
        'Agents and leads can import an existing Bruno, OpenCollection, or Postman collection with api_client_import and a repository-relative path. api_client_read/replace edits the same node shown in the UI and writes linked sources back by default; sync-status, pull, and push expose conflicts before either side is replaced. Format-native requests, folders, scripts, tests, and variables stay in ordinary project files ready for git, Bruno, Postman, and CI; Orkestrai-only runner configuration remains in the node and lossless native backup.',
      ],
      examples: [
        {
          id: 'postman',
...
Header: Authorization = Bearer {{accessToken}}`,
  ...
  useCases: [
    ...
    },
    {
      ...
      body: 'Add an API Client and import Bruno, OpenCollection YAML, Postman v2.1, Swagger 2.0, or OpenAPI 3.x. For a project that already owns Bruno/Postman tests, ask the lead or agent to use api_client_import with the repository-relative path: the same collection appears in Canvas and Workbench, and subsequent api_client_replace edits persist in the original files. Organize folders, environments, and runners, write automation and JavaScript tests with completion, run the suite, inspect sync conflicts, then commit the changed collection with the rest of the project.',
