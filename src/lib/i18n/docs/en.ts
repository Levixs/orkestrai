...
export const DOCS_EN: DocsCatalog = {
  ...
  useCases: [
    ...
    },
    {
      ...
      body: 'Enable experimental workspace sharing, start an end-to-end encrypted session, and choose a Browser/mobile or Orkestrai app invite. The web link opens the installable Remote PWA. The app invite opens the installed Orkestrai app automatically; the guest can also use Workspace → Join remote workspace and paste the invite. Both remove the secret from the URL before connecting and store a non-extractable pairing key only on that device. Approve the exact device fingerprint and choose Viewer, Collaborator, Operator, or Administrator. An Operator can hold a sanitized, traceable conversation with an agent and dictate a leader message through the host local STT; chat replies appear only when they match the exact question and provider conversation. An Administrator can also start or restore an offline agent. Raw terminal control is a separate Administrator-only switch on that device, disabled by default, limited to one responsive terminal, rate-limited, encrypted, and audited. It never grants file browsing, Portal or mobile-device viewing, or Canvas editing. Revoke a device or stop the session at any time, and inspect accepted and rejected commands in the audit trail.',
  ...
  changelog: [
    {
      ...
      items: [
        'Remote collaboration now supports traceable conversations bound to the exact question and provider session across every registered provider, leader-message dictation through the host local STT, Administrator-only start or restore, and a separately approved raw terminal that is disabled by default, responsive on phones, rate-limited, limited to one session, encrypted, and audited. Opening the terminal closes the conversation before taking over the screen.',
