...
export const DOCS_EN: DocsCatalog = {
  ...
  sections: [
    ...
    },
    {
      ...
      body: `Add Design from the Canvas toolbar to create a structured visual document stored under .orkestrai/designs in the project. Double-click its preview or use expand to open Design Mode; the same document opens as a full Workbench item. Draw frames, rectangles, ellipses, text, and Pen paths at any size. Shift-select layers to align, distribute, combine with union, subtract, intersect, or exclude, and create or release masks. Edit path points and handles; stack solid or linear/radial gradient fills and strokes; add shadows, blur, blend modes, rulers, persistent guides, and snapping. Frames support horizontal, vertical, wrapping, and grid auto layout, padding and gaps, while child constraints respond when a frame is resized. Import PNG, JPEG, WebP, GIF, or SVG by picker, clipboard, or drag and drop, then reuse the asset or export the page as SVG, PNG, JPEG, WebP, or PDF. Delete removes only selected layers, Escape cancels the active tool, and undo/redo uses typed operations. Every mutation records a revision and bounded history; revision-bound thumbnails keep large Canvas previews light. Connect the Design node to a leader or specialist so they can read and edit the exact scene graph through typed Orkestrai MCP tools. Human and agent edits use optimistic revision checks, update open editors live, and never require direct JSON rewriting.`,
    },
    {
      id: 'presets',
      title: 'Team presets',
