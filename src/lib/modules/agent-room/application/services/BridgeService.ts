...
- Tool MCP \`design_apply_operations\` — command bus completo para elementos, grupos, vetores, guias, tokens, modos, bindings, componentes, instâncias, variantes, protótipos, interações, apresentação, motion tokens, tracks e keyframes. As tools \`design_create_element\`, \`design_update_element\` e \`design_delete_element\` são atalhos para operações simples. Passe \`taskId\` quando a alteração pertence a uma task. Trabalhe no ciclo ler → alterar usando \`baseRevision\` → ler e verificar; conflito exige reler, nunca sobrescrever o trabalho humano.
...
`;
  ...
  private agentsMdBlock(): string {
    return [
      ...
      '- `orkestrai design list/read/apply` — documentos visuais nativos com elementos, vetores, grupos, tokens, modos, bindings, componentes, instancias, variantes, prototipos, interacoes, apresentacao e animacoes; use preferencialmente a tool MCP design_apply_operations (ou os atalhos design_*) e sempre leia a revisao antes/depois de alterar.',
